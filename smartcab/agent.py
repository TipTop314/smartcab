...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            # Set Epsilon to 1 for a bunch of trials
            if self.trial<300:
                self.epsilon = 1
            elif self.trial<400:
                self.epsilon = math.pow(0.1,10)
            else:
                self.epsilon = 0
            ...
            #self.epsilon = 2 - pow(B,self.trial)
    ...
    def createQ(self, state):
        ...
        if not state in self.Q:       
            self.Q[state]={x:0.0 for x in self.valid_actions}
        return
    ...
    def choose_action(self, state):
        ...
        if self.learning == True and random.random() < 1 - self.epsilon:
            maxQ = max(self.Q[state].values())
            maxActions = [k for k,v in self.Q[state].iteritems() if v == maxQ]
    ...
    def update(self):
        ...
        #Keep state, action, and reward for next time step
...
def run():
    ...
    #sim.run()
    sim.run(n_test=100, tolerance = math.pow(0.1,20))
